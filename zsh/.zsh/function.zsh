...
function rm () {
  ...
  if [ $1 = "-rf" ]; then filename=$2 ;else filename=$1 ;fi
  if ! ls $filename > /dev/null 2>&1; then return ;fi

  moved_filename=${filename##*/}

  if ls ~/.Trash/$moved_filename > /dev/null 2>&1; then
    moved_filename="$moved_filename $(date +"%k.%M.%S")"
  fi
